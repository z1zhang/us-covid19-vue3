<template>
  <div id="rankChart" style="width: 23%;height:300px" ref="main">
  </div>
</template>
...
export default {
  ...
  methods: {
    getLeastCases() {
      this.axios.post("api/top10DeathRate").then(res => {
        if ((res.data.status === 200)) {
          const json = JSON.stringify(res.data.data)
          this.rankData.labels = JSON.parse(json).map(item => item.state)
          this.rankData.datasets = JSON.parse(json).map(item => item.rate)
          this.setMapData()
        }
      })
    },
